...
class GameData {
    constructor(update_interval, enable_update) {
        this.items_game_url = 'https://raw.githubusercontent.com/SteamDatabase/GameTracking-CS2/refs/heads/master/game/csgo/pak01_dir/scripts/items/items_game.txt';
        ...
        this.csgo_english_url = 'https://raw.githubusercontent.com/SteamDatabase/GameTracking-CS2/refs/heads/master/game/csgo/pak01_dir/resource/csgo_english.txt';
    ...
    */
    addAdditionalItemProperties(iteminfo) {
        ...
        // PATCH: Ajoute un keychain si trouvé dans la description (ex: "Charm: ...")
        if ((!iteminfo.keychains || iteminfo.keychains.length === 0) && iteminfo.description) {
            const charmMatch = iteminfo.description.match(/Charm: ([^|\n]+)/);
            if (charmMatch) {
                const charmName = charmMatch[1].trim();
                // Recherche d'une image comme pour les stickers
                let imageUrl = '';
                if (this.items_game_cdn && this.items_game_cdn[charmName]) {
                    imageUrl = this.items_game_cdn[charmName];
                }
                iteminfo.keychains = [{ name: charmName, imageUrl }];
            }
        }

        // Enrichit chaque keychain avec toutes les propriétés utiles et imageUrl si possible
        for (const keychain of iteminfo.keychains || []) {
            const kit = keychainDefinitions[keychain.sticker_id];
            if (kit) {
                let name = this.csgo_english[kit.loc_name.replace('#', '')];
                if (name) keychain.name = name;
                // Ajoute imageUrl si possible (même logique que stickers)
                if (this.items_game_cdn && this.items_game_cdn[name]) {
                    keychain.imageUrl = this.items_game_cdn[name];
                }
            }
            // Les propriétés brutes (offset_x, offset_y, etc.) sont déjà présentes, on les laisse telles quelles
        }
    }
